...
// دالة أمان للتأكد من وجود مصفوفة السور ودعم الأرقام العربية واللاتينية
function toAr(num) {
  return typeof toArabicDigits === 'function' ? toArabicDigits(num) : num;
}
...
function startKhatma(days) {
  try {
    ...
    // إعادة تحميل خفيفة لضمان استقرار الواجهة بعد تفعيل البيانات
    window.location.reload();
  } catch (error) {
    console.error("خطأ أثناء بدء الختمة:", error);
    window.location.reload(); // حماية طوارئ لإعادة تشغيل الصفحة بدلاً من البياض
  }
}
...
function renderKhatma() {
  ...
  // الحماية والتأكد من وجود المتغيرات الحيوية قبل الرندرة
  const safeSurahs = typeof surahs !== 'undefined' ? surahs : [];
  const progress = safeSurahs.length > 0 ? (khatmaData.completedSurahs.length / safeSurahs.length) * 100 : 0;
  ...
  for(let i = 0; i < safeSurahs.length; i++) {
    const sPages = estimateSurahPages(safeSurahs[i].ayat, safeSurahs[i].n);
    const sStart = cumulativePages;
    const sEnd = cumulativePages + sPages;

    if(sEnd > prevTargetPages && sStart < targetPages) {
      todaySurahs.push(safeSurahs[i].name);
    }
  ...
  if(compSurahsEl && safeSurahs.length > 0) {
    let allSurahsHtml = safeSurahs.map(s => {
