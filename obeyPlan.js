...
// هيكل ومجموعات المهام الجاهزة المصنفة إيمانيًا
...
function renderObeyFollowUpMode(container) {
  ...
  tasks.forEach((task, tIdx) => {
    ...
    for (let d = 1; d <= totalDays; d++) {
      ...
      const status = obeyPlanProgress[cellKey] || '';
      let displaySymbol = '';
...
window.cycleObeyCellStatus = function(cellKey) {
  ...
  const currentStatus = obeyPlanProgress[cellKey] || '';
  let newStatus = '';
...
function calculateAndRenderObeyStats() {
  ...
  const currentDayNum = (dayDiff >= 1 && dayDiff <= totalDays) ? dayDiff : 1;
  ...
  if (motivateBadge) {
    if (currentDayDone === tasks.length && tasks.length > 0) {
      ...
      motivateBadge.innerHTML = `<span style="color:var(--text2);">ثبّت التزامك اليومي لليوم ${toAr(currentDayNum)} 🌿</span>`;
    }
  }
}
