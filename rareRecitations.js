...
// ==========================================
// تايمر النوم (Sleep Timer) - يشغل تلاوات متتابعة لحد ما تخلص المدة المحددة
// ==========================================
window.sleepTimer = {
  active: false,
  endAt: 0,        // وقت الانتهاء (timestamp بالملي ثانية)
  totalMinutes: 0,
  intervalId: null
};

// تنسيق الثواني لصيغة دقائق:ثواني
...
// إرجاع القائمة المفلترة الحالية (نفس منطق الفلترة في renderRareRecitations)
...
// تشغيل التلاوة اللي بعد التلاوة الحالية في القائمة المفلترة (بيلف على الأول لو خلصت القائمة)
...
window.startSleepTimer = function (minutes) {
  ...
  // لو مفيش تلاوة شغالة حالياً، شغل أول تلاوة في القائمة المفلترة
  if (!window.currentRareUrl || window.rareAudioPlayer.paused) {
    const list = window.getFilteredRareList();
    if (list.length > 0) {
      const startUrl = window.currentRareUrl || list[0].url;
      window.playRare(startUrl);
    }
  }
...
// بيتفحص كل ثانية: لو خلصت المدة يوقف الصوت، لو لسه فاضل وقت يحدث العداد
window.tickSleepTimer = function () {
  ...
  if (remainingMs <= 0) {
    // خلصت المدة: وقف الصوت وامسح التايمر
...
// تحديث شريط التقدم والوقت أثناء تشغيل الصوت بالثانية
window.rareAudioPlayer.ontimeupdate = () => {
  ...
  const activeTimeLabel = document.querySelector(`span[data-time-url="${window.currentRareUrl}"]`);
  if (activeTimeLabel) {
...
window.rareAudioPlayer.onplay = () => {
  ...
  // 🌟 التحديث الفوري المضمون للاسم واللوجو الكامل للتلاوات الخاشعة
  if ('mediaSession' in navigator && window.currentRareUrl) {
    ...
    // توليد المسار الكامل للأيقونة ديناميكياً لتجنب مشاكل مسارات الـ PWA
    const fullIconUrl = window.location.origin + window.location.pathname.replace(/[^\/]*$/, '') + 'icon.png';

    navigator.mediaSession.metadata = new MediaMetadata({
      title: trackName,
      artist: 'أثر', // تم تعديل الاسم إلى "أثر" بالظبط كما طلبت
...
// ==========================================
// شريط تحكم تايمر النوم (يترسم فوق القائمة)
// ==========================================
...
<button id="rare_dl_${item.url.replace(/[^a-zA-Z0-9]/g,'_')}" onclick="window.downloadRareAudio('${item.url}')" style="background:rgba(255,255,255,0.1); border:1px solid var(--border); width:42px; height:42px; border-radius:50%; color:var(--text); font-size:16px; cursor:pointer; display:flex; align-items:center; justify-content:center; transition:0.2s;" title="تحميل للاستماع أوفلاين">📥</button>          </div>
        ...
        <!-- القسم السفلي: شريط مدة الصوت والتحكم بالتجري + عرض الوقت -->
