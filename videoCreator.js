...
function vcStartAyahPlayback(onComplete) {
  ...
  // حل مشكلة CORS الحيوية لتراكات السيرفرات الخارجية
  ...
  function playAyahAtCurrentIndex() {
    ...
    audioEl.load();
    audioEl.play().catch(e => console.log("خطأ في تشغيل تراك الصوت:", e));
  }
  ...
  try {
    ...
    if (data.code === 200 && data.data && data.data.ayahs) {
      ...
      if (selectedAyahs.length > 0) {
        vcCurrentAyahText = selectedAyahs.map(a => `${a.text} ﴿${toAr(a.numberInSurah)} Bled﴾`).join(' ');
        // تصحيح بسيط للوصول إلى دالة التحويل لقاموس هندي/عربي المعتمدة لديك إن وجدت، أو مسح toAr والاعتماد على الأرقام العادية:
...
// ==========================================
// 7. الدالة العملاقة لدمج التراكات وتصدير وحفظ فيديو الريلز النهائي (MP4) المحدثة
// ==========================================
function generateFinalVideo() {
  ...
  // 1. التقاط البث الصوري من الكانفاس
  const videoStream = canvas.captureStream(30);
  const finalStream = new MediaStream();

  // إضافة تراكات الفيديو أولاً
  videoStream.getVideoTracks().forEach(track => finalStream.addTrack(track));

  // 2. 🌟 الحل الجذري لمشكلة انعدام الصوت: استخدام AudioContext لالتقاط الصوت من العنصر بشكل نظيف
  try {
    if (!vcAudioCtx) {
      vcAudioCtx = new (window.AudioContext || window.webkitAudioContext)();
      // إنشاء عقدة المصدر وعقدة الوجهة
    ...
    // حل احتياطي في حال فشل الـ Web Audio API لأي سبب
    if (typeof audioEl.captureStream === 'function') {
      ...
      } catch (e) { console.error("فشل الحل الاحتياطي أيضاً:", e); }
    }
  }

  let recordedChunks = [];
  
  // صيغ تسجيل متوافقة مع متصفحات الموبايل والكمبيوتر
  ...
  try {
    ...
    mediaRecorder.onstop = function() {
      ...
      alert("🎉 ألف مبروك! تم إنشاء مقطع الريلز وحفظه بنجاح مع الصوت! ✨");
    ...
    // حلقة الانيميشن الفريمية أثناء التصدير
    vcIsPlaying = true;
    ...
    // حماية التصدير من اللوب اللانهائي (90 ثانية كحد أقصى للريلز)
    ...
    statusEl.style.display = "none";
    alert("عذراً، هناك مشكلة في المتصفح أثناء التصدير المباشر 😔");
  }
}
